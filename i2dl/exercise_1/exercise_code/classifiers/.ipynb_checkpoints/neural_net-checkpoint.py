...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
       ...
       #########################################################################
        ...
        softmax = scores / (sums.reshape(N, 1))
       ...
       #########################################################################
        ...
        trues[np.arange(N),y] = 1
        softmax = (softmax - trues ) # / X.shape[0]
        grads['W2'] = (((np.transpose(relu)).dot(softmax))/ N) +2*reg*W2
        grads['b2'] = softmax.sum(axis=0) / N
        
        after_relu = softmax.dot(np.transpose(W2))
        before_relu = (first_layer > 0) * after_relu  # or relu >0
        grads['W1'] = (np.transpose(X).dot(before_relu) / N) +2*reg*W1
        grads['b1'] = np.sum(before_relu, axis = 0) / N
...
def neuralnetwork_hyperparameter_tuning(X_train, y_train, X_val, y_val):
    ...
    combinations = np.array(np.meshgrid(learning_rates,regularization_strengths,hidden_size)).T.reshape(-1,3)
    
    for pair in combinations:
        model = TwoLayerNet(input_size, int(pair[2]), output_size, std=1e-4)
        model_hist = model.train(X_train, y_train, X_val, y_val, learning_rate=pair[0],
                                                    reg=pair[1], 
                                                    learning_rate_decay=0.95,
                                                    num_iters=2000, batch_size=200, verbose=False)
        y_train_pred = model.predict(X_train)
        train_acc = model_hist['train_acc_history']
        y_val_pred = model.predict(X_val)
        val_acc = model_hist['val_acc_history'][-1]
        if val_acc > best_val:
            best_val = val_acc
            best_net = model
        print("Parameters lr = {}, reg = {}, num_layer = {} : validation_accuracy = {}".format(pair[0],pair[1],pair[2],val_acc))
    print("Validation accuracy of the best model: {}".format(best_val))
