...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3 * 32 * 32, hidden_dim=100, num_classes=10,
        ...
        self.params['W1'] = weight_scale*np.random.randn(input_dim, hidden_dim)  # + mean ???
        self.params['W2'] = weight_scale*np.random.randn(hidden_dim, num_classes) # + mean ???
    ...
    def loss(self, X, y=None):
        ...
        scores, cache_3 = affine_forward(out_2, W2, b2)
        ...
        loss, deriv_scores = softmax_loss(scores, y)
        loss += reg*(np.sum(W1*W1)+np.sum(W2*W2))*0.5
        
        dx_2, dw_2, db_2 = affine_backward(deriv_scores, cache_3)
        before_relu = relu_backward(dx_2, cache_2)
        dx_1, dw_1, db_1 = affine_backward(before_relu, cache_1)
    
        grads = {"W1": dw_1+reg*W1, "W2": dw_2+reg*W2, "b1": db_1, "b2":db_2}
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3 * 32 * 32, num_classes=10,
        ...
        ############################################################################
        new_dim = input_dim
        for i,hidden_size in enumerate(hidden_dims,1):
            self.params['W'+str(i)] = weight_scale*np.random.randn(new_dim, hidden_size)  # + mean ???
            self.params['b'+str(i)] = np.zeros(hidden_size)      
            new_dim = hidden_size
        self.params['W'+str(self.num_layers)] = weight_scale*np.random.randn(new_dim, num_classes) # + mean ???
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1,self.num_layers):    
            input_x, cache[i] = affine_relu_forward(input_x, self.params['W'+str(i)], self.params['b'+str(i)])
        scores, cache[self.num_layers] = affine_forward(input_x, self.params['W'+str(self.num_layers)], self.params['b'+str(self.num_layers)])
        ...
        dx_last, dw_last, db_last = affine_backward(deriv_scores, cache[self.num_layers])                         
        grads["W" + str(self.num_layers)] = dw_last + self.reg*self.params["W"+ str(self.num_layers)]
        grads["b" + str(self.num_layers)] = db_last
        
        deriv_layer = dx_last
        for i in range(self.num_layers-1, 0, -1):
            dx_layer, dw_layer, db_layer = affine_relu_backward(deriv_layer, cache[i])
            grads["W" + str(i)] = dw_layer + self.reg*self.params["W"+ str(i)]
            grads["b" + str(i)] = db_layer
            deriv_layer = dx_layer
